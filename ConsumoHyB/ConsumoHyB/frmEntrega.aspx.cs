...
namespace ConsumoHyB
{
    public partial class frmEntrega : System.Web.UI.Page
    {
        ...
        {
            try { gvEntrega.DataSource = servicio.MostrarEntregaTodo(); gvEntrega.DataBind(); }
            catch (Exception ex) { Response.Write(ex.ToString()); }
        }
        ...
        protected void btnActualizar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                obj.responsable = txtResponsable.Text;
                obj.estado = chkEstado.Checked;
                bool res = servicio.ActualizarEntrega(obj);
                ScriptManager.RegisterStartupScript(this, GetType(), "m", "alert('" + (res ? "Operacion correcta" : "No se pudo") + "');", true);
                Mostrar(); Limpiar();
            }
            catch (Exception ex) { Response.Write(ex.ToString()); }
        ...
        protected void btnEliminar_Click(object sender, EventArgs e)
        {
            try
            {
                var obj = new EntregaBO();
                obj.codigo = Convert.ToInt32(txtCod.Text);
                bool res = servicio.EliminarEntrega(obj);
                ScriptManager.RegisterStartupScript(this, GetType(), "m", "alert('" + (res ? "Operacion correcta" : "No se pudo") + "');", true);
                Mostrar(); Limpiar();
            }
            catch (Exception ex) { Response.Write(ex.ToString()); }
        ...
        protected void btnHabilitar_Click(object sender, EventArgs e)
        {
            try
            {
                var obj = new EntregaBO();
                obj.codigo = Convert.ToInt32(txtCod.Text);
                bool res = servicio.HabilitarEntrega(obj);
                ScriptManager.RegisterStartupScript(this, GetType(), "m", "alert('" + (res ? "Operacion correcta" : "No se pudo") + "');", true);
                Mostrar(); Limpiar();
            }
            catch (Exception ex) { Response.Write(ex.ToString()); }
        ...
        protected void gvEntrega_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Seleccionar")
            {
                int i = Convert.ToInt32(e.CommandArgument);
                GridViewRow f = gvEntrega.Rows[i];
                ...
                txtResponsable.Text = f.Cells[3].Text.Trim();
