...
namespace ConsumoHyB
{
    public partial class frmPedido : System.Web.UI.Page
    {
        ...
        private void CargarCombos()
        {
            try
            {
                ddlCodCliente.DataSource = servicio.MostrarClienteTodo();
                ddlCodCliente.DataValueField = "codigo"; ddlCodCliente.DataTextField = "razonSocial"; ddlCodCliente.DataBind();
                ddlCodEstado.DataSource = servicio.MostrarEstadoPedidoTodo();
                ddlCodEstado.DataValueField = "codigo"; ddlCodEstado.DataTextField = "nombre"; ddlCodEstado.DataBind();
                ...
                ddlDCodTipoTejido.DataSource = servicio.MostrarTipoTejidoTodo();
                ddlDCodTipoTejido.DataValueField = "codigo"; ddlDCodTipoTejido.DataTextField = "nombre"; ddlDCodTipoTejido.DataBind();
        ...
        private DataTable GetDetalle()
        {
            ...
            dt.Columns.Add("codTipoTejido");
            dt.Columns.Add("codTipoTejidoTexto");
        ...
        protected void btnAgregarDetalle_Click(object sender, EventArgs e)
        {
            try
            {
                DataTable dt = GetDetalle();
                DataRow r = dt.NewRow();
                r["codTipoTejido"] = ddlDCodTipoTejido.SelectedValue;
                r["codTipoTejidoTexto"] = ddlDCodTipoTejido.SelectedItem.Text;
        ...
        {
            try { gvPedido.DataSource = servicio.MostrarPedidoTodo(); gvPedido.DataBind(); }
            catch (Exception ex) { Response.Write(ex.ToString()); }
        }
        ...
        protected void btnRegistrar_Click(object sender, EventArgs e)
        {
            try
            {
                var cab = new PedidoBO();
                cab.codCliente = Convert.ToInt32(ddlCodCliente.SelectedValue);
                cab.codEstado = Convert.ToInt32(ddlCodEstado.SelectedValue);
                ...
                cab.estado = true;
                int id = servicio.RegistrarPedidoRetornaId(cab);
                if (id > 0)
                {
                    foreach (DataRow r in GetDetalle().Rows)
                    {
                        var d = new DetallePedidoBO();
                        d.codPedido = id;
                        d.codTipoTejido = Convert.ToInt32(r["codTipoTejido"]);
                        d.codUnidad = Convert.ToInt32(r["codUnidad"]);
                        d.cantidad = Convert.ToDecimal(r["cantidad"]);
                        d.precio = Convert.ToDecimal(r["precio"]);
                        d.estado = true;
                        servicio.RegistrarDetallePedido(d);
                    }
                }
        ...
        protected void btnActualizar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                cab.total = Convert.ToDecimal(lblTotal.Text);
                cab.estado = true;
                bool res = servicio.ActualizarPedido(cab);
        ...
        protected void btnEliminar_Click(object sender, EventArgs e)
        {
            try
            {
                var cab = new PedidoBO();
                cab.codigo = Convert.ToInt32(txtCod.Text);
                bool res = servicio.EliminarPedido(cab);
        ...
        protected void btnHabilitar_Click(object sender, EventArgs e)
        {
            try
            {
                var cab = new PedidoBO();
                cab.codigo = Convert.ToInt32(txtCod.Text);
                bool res = servicio.HabilitarPedido(cab);
                ScriptManager.RegisterStartupScript(this, GetType(), "m", "alert('" + (res ? "Operacion correcta" : "No se pudo") + "');", true);
                Limpiar(); Mostrar();
            }
            catch (Exception ex) { Response.Write(ex.ToString()); }
        ...
        protected void gvPedido_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                int i = Convert.ToInt32(e.CommandArgument);
                GridViewRow f = gvPedido.Rows[i];
                txtCod.Text = f.Cells[0].Text;
                try { ddlCodCliente.SelectedValue = f.Cells[2].Text.Trim(); } catch { }
                try { ddlCodEstado.SelectedValue = f.Cells[3].Text.Trim(); } catch { }
                try { ddlCodUsuario.SelectedValue = f.Cells[4].Text.Trim(); } catch { }
                try { ddlCodMoneda.SelectedValue = f.Cells[5].Text.Trim(); } catch { }
                int cod = Convert.ToInt32(txtCod.Text);
                DataTable dt = GetDetalle(); dt.Rows.Clear();
                foreach (var d in servicio.MostrarDetallePedidoXPedido(cod))
                {
                    DataRow r = dt.NewRow();
                    r["codTipoTejido"] = d.codTipoTejido; r["codTipoTejidoTexto"] = d.codTipoTejido.ToString();
