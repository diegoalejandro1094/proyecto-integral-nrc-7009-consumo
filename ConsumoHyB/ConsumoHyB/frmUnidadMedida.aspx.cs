...
namespace ConsumoHyB
{
    public partial class frmUnidadMedida : System.Web.UI.Page
    {
        ...
        {
            try { gvUnidadMedida.DataSource = servicio.MostrarUnidadMedidaTodo(); gvUnidadMedida.DataBind(); }
        ...
        protected void btnRegistrar_Click(object sender, EventArgs e)
        {
            try
            {
            var obj = new UnidadMedidaBO();
                obj.nombre = txtNombre.Text;
                obj.abreviatura = txtAbreviatura.Text;
                obj.estado = chkEstado.Checked;
                bool res = servicio.RegistrarUnidadMedida(obj);
        ...
        protected void btnActualizar_Click(object sender, EventArgs e)
        {
            ...
            var obj = new UnidadMedidaBO();
                ...
                obj.abreviatura = txtAbreviatura.Text;
                obj.estado = chkEstado.Checked;
                bool res = servicio.ActualizarUnidadMedida(obj);
                ScriptManager.RegisterStartupScript(this, GetType(), "m", "alert('" + (res ? "Operacion correcta" : "No se pudo") + "');", true);
                Mostrar(); Limpiar();
            }
            catch (Exception ex) { Response.Write(ex.ToString()); }
        ...
        protected void btnEliminar_Click(object sender, EventArgs e)
        {
            try
            {
                var obj = new UnidadMedidaBO();
                obj.codigo = Convert.ToInt32(txtCod.Text);
                bool res = servicio.EliminarUnidadMedida(obj);
                ScriptManager.RegisterStartupScript(this, GetType(), "m", "alert('" + (res ? "Operacion correcta" : "No se pudo") + "');", true);
                Mostrar(); Limpiar();
            }
            catch (Exception ex) { Response.Write(ex.ToString()); }
        ...
        protected void btnHabilitar_Click(object sender, EventArgs e)
        {
            try
            {
                var obj = new UnidadMedidaBO();
                obj.codigo = Convert.ToInt32(txtCod.Text);
                bool res = servicio.HabilitarUnidadMedida(obj);
                ScriptManager.RegisterStartupScript(this, GetType(), "m", "alert('" + (res ? "Operacion correcta" : "No se pudo") + "');", true);
                Mostrar(); Limpiar();
            }
            catch (Exception ex) { Response.Write(ex.ToString()); }
        ...
        protected void gvUnidadMedida_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Seleccionar")
            {
                int i = Convert.ToInt32(e.CommandArgument);
                GridViewRow f = gvUnidadMedida.Rows[i];
                txtCod.Text = f.Cells[0].Text;
                txtNombre.Text = f.Cells[1].Text;
                txtAbreviatura.Text = f.Cells[2].Text;
